...
var (
	...
	// ourselves to other LBC peers.
	UserAgentName = "lbc-neutrino"

	// UserAgentVersion is the user agent version and is used to help
	// identify ourselves to other LBC peers.
	...
	// supported by outbound peers. Witness is optional so a custom daemon
	// that serves compact filters but does not advertise SFNodeWitness
	// still works.
...
type Config struct {
	...
	// DisableDNSSeed skips DNS seed lookups. Use with ConnectPeers/AddPeers
	// when talking only to a user-supplied daemon.
	DisableDNSSeed bool

	// DNSSeeds, if non-empty, replaces ChainParams.DNSSeeds. Hosts only;
	// default P2P port from chain params is used.
	DNSSeeds []string
...
func NewChainService(cfg Config) (*ChainService, error) {
	...
	s := ChainService{
		...
		disableDNSSeed:    cfg.DisableDNSSeed,
	}
	if len(cfg.DNSSeeds) > 0 {
		seeds := make([]chaincfg.DNSSeed, 0, len(cfg.DNSSeeds))
		for _, host := range cfg.DNSSeeds {
			seeds = append(seeds, chaincfg.DNSSeed{Host: host, HasFiltering: true})
		}
		s.chainParams.DNSSeeds = seeds
	}
...
func (s *ChainService) peerHandler() {
	...
	if !DisableDNSSeed && !s.disableDNSSeed {
